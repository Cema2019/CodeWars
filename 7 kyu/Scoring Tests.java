...
public class Kata {
    public static int sol(int[] arr, int r, int o, int w){
        int right = (int) Arrays.stream(arr).filter(x -> x == 0).count();
        int omitted = (int) Arrays.stream(arr).filter(x -> x == 1).count();
        int wrong = (int) Arrays.stream(arr).filter(x -> x == 2).count();
        return (right * r) + (omitted * o) - (wrong * w);
    }
  }
